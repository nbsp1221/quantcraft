...
class Strategy[ConfigT: StrategyConfig](ABC):
    ...
    def __init__(self, config: ConfigT | None = None) -> None:
        ...
        object.__setattr__(self, "config", materialized)
        object.__setattr__(self, "_config_initialized", True)
        self._reset_runtime_state()

    def __setattr__(self, name: str, value: object) -> None:
        if name == "config" and getattr(self, "_config_initialized", False):
            raise StrategyConfigMutationError("strategy config cannot be reassigned")
        object.__setattr__(self, name, value)

    def __delattr__(self, name: str) -> None:
        if name == "config" and getattr(self, "_config_initialized", False):
            raise StrategyConfigMutationError("strategy config cannot be deleted")
        object.__delattr__(self, name)

    def _materialize_config(self, config: ConfigT | None) -> StrategyConfig:
        if config is None:
            return self.config_type()
        if type(config) is not self.config_type:
            raise StrategyConfigValidationError(
                f"{type(self).__name__} expected config {self.config_type.__name__}"
            )
        return config
...
def _generic_config_type(cls: type[Strategy[Any]]) -> type[StrategyConfig] | None:
    for base in getattr(cls, "__orig_bases__", ()):
        if get_origin(base) is Strategy:
            args = get_args(base)
            if len(args) == 1:
                return _validate_config_type(args[0], cls)
    return None


def _inherited_config_type(cls: type[Strategy[Any]]) -> type[StrategyConfig] | None:
    for base in cls.__mro__[1:]:
        config_type = getattr(base, "config_type", None)
        if isinstance(config_type, type) and issubclass(config_type, StrategyConfig):
            return config_type
    return None


def _validate_config_type(candidate: object, cls: type[object]) -> type[StrategyConfig]:
    if isinstance(candidate, type) and issubclass(candidate, StrategyConfig):
        return candidate
    raise StrategyConfigDeclarationError(
        f"{cls.__name__}.config_type must be a StrategyConfig subclass"
    )


__all__ = ["PositionView", "Strategy"]
