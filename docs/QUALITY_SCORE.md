...
| --- | --- | --- | --- |
| data | B | The shipped historical ingestion surface now lives under `src/quantcraft/data/` with unit coverage in `tests/unit/data/` and exchange-backed coverage in `tests/unit/market_data/test_exchange_fetch_ohlcv.py`. | Data is now a real implemented bounded context, but it remains intentionally historical-only and narrower than a full data platform. |
...
| verification | B | `scripts/repo_check.py`, `scripts/coverage_check.py`, `scripts/update_quality_score.py`, `tests/structure/repo/test_quality_scaffolding.py`, and `pyproject.toml` exercise the repo-local harness contract. | Verification now includes a source-only coverage floor, but branch coverage and changed-line coverage are still out of scope. |
