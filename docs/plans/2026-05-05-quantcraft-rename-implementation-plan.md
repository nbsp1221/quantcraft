# Quantcraft Rename Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to
> implement this plan task-by-task.

**Goal:** Rename the public project, installable distribution, import package,
and GitHub repository to Quantcraft/`quantcraft` before the first public beta
release.

**Architecture:** Treat this as a pre-release hard rename, not a compatibility
migration. Move the single installable engine package to `src/quantcraft`, update
...
must teach and verify Quantcraft only. Do not preserve a rename narrative or
compatibility story.
...
- Goal:
  - Rename the package and public project surface to Quantcraft before any public
...
- Why these are governing:
  ...
  - `docs/product-specs/public-beta-documentation.md` controls the public docs
    and public import examples that must teach `quantcraft`.
- In-repo scope:
  - Rename the source package directory to `src/quantcraft`.
...
- Approval record, if required:
  ...
  - Mechanical import-path edits may touch files under `src/quantcraft/trading`
...
- Verification commands:
  ...
  - `uv run poe verify`
- Success criteria:
  - `import quantcraft` works from the source checkout and from the built wheel.
  - Documented public imports use `quantcraft.data`, `quantcraft.backtest`, and
    `quantcraft.research`.
  ...
  - GitHub repository name is `nbsp1221/quantcraft`, and local `origin` points at
    it.
...
- `pyproject.toml` must declare:
  - `[project].name = "quantcraft"`
  - `packages = ["quantcraft"]`
  - `tool.mypy.packages = ["quantcraft"]`
  - `tool.coverage.report.include = ["src/quantcraft/*"]`
  - project URLs under `https://github.com/nbsp1221/quantcraft`
  - changelog URL pointing at `blob/main/CHANGELOG.md`
- The installable package root must be `src/quantcraft`.
...
- `src/quantcraft/_repo_tools.py` embeds package-root assumptions for:
  - import-boundary parsing
  - architecture checks
  - package topology checks
  - CCXT import checks
  - public docs checks
- All tracked notebooks must import `quantcraft` in code cells after cleanup.
- `dist/` currently contains ignored old build artifacts and must be cleaned
  before validating wheel names.

## Rename Policy

- Use `Quantcraft` for human-facing title case.
- Use `quantcraft` for package, distribution, import path, directory name, and
...
- Do not add text saying Quantcraft was renamed from the old project name.
...
- Evaluator-owned done contract for this slice:
  ...
  - Public docs, examples, package metadata, and built artifacts consistently
    use Quantcraft/`quantcraft`.
  - GitHub repository and local `origin` use `nbsp1221/quantcraft`.
...
- Auto-fail conditions:
  - Any `quantcraft` compatibility package, shim, or transition narrative is added.
  - Any runtime logic changes beyond import paths are made under `trading` or
    `execution`.
  - `pyproject.toml` still names the distribution `quantcraft`.
  - Built wheel fails to import `quantcraft`.
  - Public docs still instruct users to install or import `quantcraft`.
...
assert urls["Changelog"] == "https://github.com/nbsp1221/quantcraft/blob/main/CHANGELOG.md"
```
...
- Change wheel glob from `quantcraft-*.whl` to `quantcraft-*.whl`.
- Change the assertion message to mention a `quantcraft` wheel.
- Change `import quantcraft` to `import quantcraft`.
- Change all `quantcraft.*` module strings to `quantcraft.*`.
...
- Change `import quantcraft` to `import quantcraft`.
- Change all `importlib.import_module("quantcraft...")` calls to
  `importlib.import_module("quantcraft...")`.
- Change old forbidden module strings from `quantcraft...` to `quantcraft...`.
...
**Files:**
- Move: existing source package root to `src/quantcraft/`
- Modify: every moved Python file under `src/quantcraft/`
...
Use `git mv` so the existing source package root becomes `src/quantcraft`.
...
from quantcraft._notebook_tools import ...
```
...
Changelog = "https://github.com/nbsp1221/quantcraft/blob/main/CHANGELOG.md"
```
...
- The root package entry is `name = "quantcraft"`.
- Dependency versions should not churn without reason.

**Step 3: Update repo tools**

In `src/quantcraft/_repo_tools.py`, ensure all hard-coded package assumptions use
`quantcraft`, `src/quantcraft`, and `quantcraft.*` sample imports.
...
When tests create fake files under `tmp_path / "src" / "quantcraft"`, move the
fixture path to `tmp_path / "src" / "quantcraft"` and update embedded import
strings accordingly.
...
- Repo check passes with Quantcraft package root and public docs.

### Task 4: Update Test Suite Imports And Architecture Fixtures

**Files:**
- Modify: all Python files under `tests/` that import or mention `quantcraft`
...
paths and forbidden legacy-module examples use `src/quantcraft` and
`quantcraft.*`. Keep the same boundary intent; this task is a rename, not a
behavior change.
...
- Modify: `SECURITY.md`
- Modify: `AGENTS.md`
- Modify: `docs/design-docs/quantcraft-architecture.md`
- Move or replace: `docs/design-docs/quantcraft-architecture.md` if the team
...
- `Quantcraft` to `Quantcraft`
- `quantcraft` to `quantcraft`
- `uv add quantcraft==0.1.0b1` to `uv add quantcraft==0.1.0b1`
- `python -m pip install quantcraft==0.1.0b1` to
  `python -m pip install quantcraft==0.1.0b1`
- public imports from `quantcraft.*` to `quantcraft.*`
...
`src/quantcraft` as the installable engine package:
...
- Rename `docs/design-docs/quantcraft-architecture.md` to
  `docs/design-docs/quantcraft-architecture.md`.
...
- If this creates too much historical churn, keep the old filename for one
  commit but change the title/content to Quantcraft and add a follow-up cleanup
...
- beta-scope copy if it names Quantcraft

Update `docs/product-specs/public-beta-documentation.md` so its external
contracts require `quantcraft.data`, `quantcraft.backtest`, and
`quantcraft.research`.
...
Bulk-edit old completed implementation records too; the repository should not keep tracked `quantcraft` text.

**Step 5: Update notebooks**

Edit tracked notebook source cells to import `quantcraft` instead of
`quantcraft`:
...
- Notebook validation executes with `quantcraft` imports.
...
- `dist/quantcraft-0.1.0b1.tar.gz`
- `dist/quantcraft-0.1.0b1-py3-none-any.whl`
- No newly built `quantcraft-*` artifacts.
...
wheel = next(Path("dist").glob("quantcraft-*.whl"))
...
print(any(name.startswith("quantcraft/") for name in names))
print(any(name.startswith("quantcraft/") for name in names))
PY
```
...
```bash
rg -n "quantcraft|Quantcraft|QUANTCRAFT" . \
  --glob '!dist/**' \
  --glob '!.venv/**' \
  --glob '!.git/**' \
  --glob '!.mypy_cache/**' \
...
```bash
gh api --method PATCH /repos/nbsp1221/quantcraft -f name=quantcraft
```

Expected:

- GitHub repository is renamed to `nbsp1221/quantcraft`.
...
```bash
git remote set-url origin https://github.com/nbsp1221/quantcraft.git
git remote -v
gh repo view nbsp1221/quantcraft --json nameWithOwner,defaultBranchRef,viewerPermission
```

Expected:

- `origin` points at `https://github.com/nbsp1221/quantcraft.git`.
- `gh repo view` reports `nameWithOwner` as `nbsp1221/quantcraft`.
...
- any historical `quantcraft` references deliberately left behind
- final disposition
...
- Verification evidence:
  ...
  - `uv build` produced `dist/quantcraft-0.1.0b1.tar.gz` and
    `dist/quantcraft-0.1.0b1-py3-none-any.whl`.
  - `uvx twine check dist/*` passed for the sdist and wheel.
  - Wheel metadata inspection reported package name `quantcraft`, version
    `0.1.0b1`, package contents under `quantcraft/`, and no package contents
  ...
  - `uv run poe verify` passed: Ruff, mypy, pytest `625 passed, 4 skipped`,
    ...
    successfully.
  - GitHub repository rename completed; `gh repo view nbsp1221/quantcraft`
    reports `nameWithOwner` as `nbsp1221/quantcraft`, default branch `main`, and
    viewer permission `ADMIN`.
  - Local `origin` now points at `https://github.com/nbsp1221/quantcraft.git`.
